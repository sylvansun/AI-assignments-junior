...
def white_space_tokenize(corpus):
    ...
    tokeneds = [
        list(filter(lambda tkn: len(tkn) > 0, _splitor_pattern.split(_digit_pattern.sub("N", stc.lower()))))
        for stc in corpus
    ]
...
# 编写所需的其他函数
def get_bigram_freq(bpe_vocab: dict):
    """
    ...
    # TODO: 完成函数体（1分）
    for word, f in bpe_vocab.items():
        word = word.split()
        for i in range(len(word) - 1):
            bigram = (word[i], word[i + 1])
            bigram_freq[bigram] = bigram_freq.get(bigram, 0) + f
...
def refresh_bpe_vocab_by_merging_bigram(bigram, old_bpe_vocab):
    ...
    # TODO: 完成函数体（1分）
    for word, f in old_bpe_vocab.items():
        word = word.replace(" ".join(bigram), "".join(bigram))
        new_bpe_vocab[word] = new_bpe_vocab.get(word, 0) + f
...
def get_bpe_tokens(bpe_vocab):
    ...
    for word, f in bpe_vocab.items():
        for w in word.split():
            bpe_dict[w] = bpe_dict.get(w, 0) + f
    for k, v in bpe_dict.items():
        bpe_tokens.append((k, v))
    bpe_tokens.sort(key=lambda x: len(x[0].replace("</w>", "A")), reverse=True)
...
def print_bpe_tokenize(word, bpe_tokens):
    ...
    def bpe_tokenize(sub_word: str):
        if len(sub_word) == 0:
            return ""
        for bpe_token, _ in bpe_tokens:
            if bpe_token in sub_word:
                idx = sub_word.find(bpe_token)
                return bpe_tokenize(sub_word[:idx]) + bpe_token + " " + bpe_tokenize(sub_word[idx + len(bpe_token) :])
        return "<unknown>"
...
if __name__ == "__main__":
    ...
    for i in range(training_iter_num):
        ...
        bigram_freq_list = sorted(list(bigram_freq.items()), reverse=True, key=lambda x: x[1])
        bigram = bigram_freq_list[0][0]
