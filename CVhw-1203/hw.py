...
if __name__ == '__main__':
    ...
    for i in range(data.train_images.shape[0]): # shape[0] is 6400 
        ...
        vec_dict[data.train_lb[i]]['kp'].extend(list(kp_vector))
        vec_dict[data.train_lb[i]]['des'].extend(map(lambda x:np.squeeze(x), np.split(des_vector, des_vector.shape[0], axis=0)))
    print("sift done")
    ...
    N_clusters = 1000
    kmeans = KMeans(n_clusters=N_clusters, max_iter=300, random_state=0).fit(vec_list)
    print("kmeans done")
    ...
    for i in range(num_test_images):
        ...
        pred = kmeans.predict(np.float64(des_vector))
        for category in range(N_clusters):
            hist_test_vector[i][category] = np.sum(pred == category)
