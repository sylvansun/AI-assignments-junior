                               ...
                               struct page *page)
{
    ...
    //delete the page from current free list
    list_del(&page->node);
    ...
    while(page->order > order){
        ...
        //always need to add buddy to free list, might need to split original chunk recursively
        list_add(&buddy->node, &pool->free_lists[buddy->order].free_list);
        pool->free_lists[buddy->order].nr_free++;
    }
    
    //mark the page splited as allocated before return
    page->allocated = 1;
    return page;
        /* LAB 2 TODO 2 END */
}
...
struct page *buddy_get_pages(struct phys_mem_pool *pool, u64 order)
{
    ...
    while(pool->free_lists[curr_order].nr_free == 0){
        curr_order++;
    }
...
static struct page *merge_page(struct phys_mem_pool *pool, struct page *page)
{
    ...
    //check if current page can be merged
    if(page->order >= BUDDY_MAX_ORDER-1 || page->allocated){
        return page;
    }
    //check if its buddy page can be merged
    struct page *buddy = get_buddy_chunk(pool,page);
    if(buddy == NULL || buddy->allocated || buddy->order != page->order){
        return page;
    }
    
    //after merge, the page must be pointing lower address
    if(page > buddy){
        struct page *tmp = buddy;
        buddy = page;
        page = tmp;
    }
    buddy->allocated = 1;//this might not be necessary but we still modify it for safety...
...
void buddy_free_pages(struct phys_mem_pool *pool, struct page *page)
{
    ...
    //if the page has been allocated, just return
    if(page->allocated == 0){
        return;
    }
    ...
    //once a page is freed, always need to check if we can merge with its buddy(recursively)
